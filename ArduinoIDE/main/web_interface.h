...
String getWebInterfaceHTML() {
  ...
  html += ".playlist-editor { max-height: 520px; overflow:auto; border:1px solid #233; padding:8px; background:#050608; border-radius:8px; }";
  html += ".playlist-editor table { width:100%; border-collapse:collapse; }";
  html += ".playlist-editor th, .playlist-editor td { padding:8px; border-bottom:1px solid rgba(255,255,255,0.04); }";
  html += ".playlist-editor tr { cursor: pointer; transition: background 0.2s; }";
  html += ".playlist-editor tr:hover { background: rgba(0,255,255,0.1); }";
  html += ".small { padding:4px 8px; font-size:13px; border-radius:6px; }";
  ...
  html += "function safeFetch(path, callback, opts){ ";
  html += " fetch(path, opts).then(r=>{ if(!r.ok) return r.text().then(t=>{ throw t }); return r.text(); }).then(data=>{ if(callback) callback(data); }).catch(err=>{ console.log('fetch err',err); if(callback) callback('{}'); }); }";
  
  html += "function updatePlayer(){ safeFetch('/api/player', function(data){ try{ var p = JSON.parse(data); ";
  ...
  html += "document.querySelector('.volume-control input').value = p.volume; }catch(e){} }); }";
  
  html += "function updateStreams(){ safeFetch('/api/streams', function(data){ try{ var streams = JSON.parse(data); ";
  html += "var html = ''; streams.forEach(function(s){ var activeClass = s.playing ? ' playing' : ''; html += '<div class=\"station-item'+activeClass+'\" onclick=\"selectStream('+s.idx+')\">'; html += '<span class=\"station-bullet\"></span>' + s.name + '</div>'; }); document.getElementById('streamsList').innerHTML = html; }catch(e){} }); }";
  
  html += "function updateBackgrounds(){ safeFetch('/api/backgrounds', function(data){ try{ var bgs = JSON.parse(data); var html = ''; bgs.forEach(function(bg){ var selClass = bg.selected ? ' selected' : ''; html += '<div class=\"file-row\">'; html += '<button class=\"file-btn'+selClass+'\" onclick=\"selectBg(\\''+bg.name+'\\')\">🖼️ '+bg.name+'</button>'; html += '<button class=\"small\" onclick=\"downloadFile(\\''+bg.name+'\\')\">⬇️</button>'; html += '<button class=\"delete-btn\" onclick=\"deleteBg(\\''+bg.name+'\\')\">🗑️</button></div>'; }); document.getElementById('bgList').innerHTML = html; }catch(e){ } }); }";

  html += "function updatePlaylists(){ safeFetch('/api/playlists', function(data){ try{ var pls = JSON.parse(data); var html = ''; pls.forEach(function(pl){ var selClass = pl.selected ? ' selected' : ''; html += '<div class=\"file-row\">'; html += '<button class=\"file-btn'+selClass+'\" onclick=\"selectPlaylist(\\''+pl.name+'\\')\">📋 '+pl.name+'</button>'; html += '<button class=\"small\" onclick=\"downloadFile(\\''+pl.name+'\\')\">⬇️</button>'; html += '<button class=\"small\" onclick=\"editPlaylist(\\''+pl.name+'\\')\">✏️ Edit</button>'; html += '<button class=\"delete-btn\" onclick=\"deletePlaylist(\\''+pl.name+'\\')\">🗑️</button>'; html += '</div>'; }); document.getElementById('plList').innerHTML = html; }catch(e){} }); }";

  html += "function updateNetworkInfo(){ safeFetch('/api/network', function(data){ try{ var n = JSON.parse(data); ";
  ...
  html += "document.getElementById('netFSFree').innerText = n.fsFree + ' KB'; }catch(e){} }); }";
  ...
  html += "function downloadFile(name){ if(!name.startsWith('/')) name = '/' + name; window.open('/download?file=' + encodeURIComponent(name), '_blank'); }";

  // ---------------- Playlist Editor ----------------
  html += "let editFilename = '';";
  html += "let selectedRow = null;";
  html += "function editPlaylist(name){ editFilename = name; showSection('playlistEditor'); document.getElementById('editorTitle').innerText = name; loadPlaylistData(name); }";
  html += "function loadPlaylistData(name){ safeFetch('/viewplaylist?pl='+encodeURIComponent(name), function(data){ try{ var arr = JSON.parse(data); var tbody = document.querySelector('#plEditorTable tbody'); tbody.innerHTML = ''; arr.forEach(function(row){ tbody.appendChild(makeRow(row.name, row.url)); }); }catch(e){ alert('Ошибка чтения плейлиста'); } }); }";
  html += "function makeRow(name, url){ var tr = document.createElement('tr'); tr.onclick = function(){ selectRow(tr); }; var td1 = document.createElement('td'); td1.contentEditable = true; td1.innerText = name || ''; td1.style.padding='6px'; tr.appendChild(td1); var td2 = document.createElement('td'); td2.contentEditable = true; td2.innerText = url || ''; td2.style.padding='6px'; tr.appendChild(td2); var td3 = document.createElement('td'); td3.style.whiteSpace='nowrap'; td3.style.padding='6px'; var del = document.createElement('button'); del.className='small'; del.style.background='#ff6b6b'; del.innerText='❌'; del.onclick = function(e){ e.stopPropagation(); if(confirm('Удалить строку?')) tr.remove(); }; td3.appendChild(del); tr.appendChild(td3); return tr; }";
  html += "function selectRow(row){ if(selectedRow) selectedRow.style.background=''; selectedRow = row; selectedRow.style.background='rgba(0,255,255,0.2)'; }";
  html += "function moveUp(){ if(!selectedRow) return alert('Выберите строку для перемещения'); var prev = selectedRow.previousElementSibling; if(prev){ selectedRow.parentNode.insertBefore(selectedRow, prev); ensureVisible(selectedRow); } }";
  html += "function moveDown(){ if(!selectedRow) return alert('Выберите строку для перемещения'); var next = selectedRow.nextElementSibling; if(next){ selectedRow.parentNode.insertBefore(next, selectedRow); ensureVisible(selectedRow); } }";
  html += "function addRow(){ var tbody = document.querySelector('#plEditorTable tbody'); var tr = makeRow('Новая станция','http://'); tbody.appendChild(tr); ensureVisible(tr); selectRow(tr); }";
  html += "function ensureVisible(row){ if(!row) return; var container = document.querySelector('.playlist-editor'); var r = row.getBoundingClientRect(); var c = container.getBoundingClientRect(); if(r.top < c.top) container.scrollTop -= (c.top - r.top) + 6; else if(r.bottom > c.bottom) container.scrollTop += (r.bottom - c.bottom) + 6; }";
  html += "function savePlaylist(){ if(!editFilename) return alert('No file'); var rows = document.querySelectorAll('#plEditorTable tbody tr'); var arr = []; for(var i=0;i<rows.length;i++){ var tds = rows[i].querySelectorAll('td'); var name = tds[0].innerText.trim(); var url = tds[1].innerText.trim(); arr.push({name:name,url:url}); } var body = JSON.stringify(arr); safeFetch('/saveplaylist?pl='+encodeURIComponent(editFilename), function(data){ alert('Saved'); updatePlaylists(); showSection('upload'); }, { method:'POST', headers: {'Content-Type':'application/json'}, body: body }); }";
  // ---------------- end Playlist Editor ----------------

  html += "setInterval(function(){ if(document.getElementById('network').classList.contains('active')) updateNetworkInfo(); if(document.getElementById('player').classList.contains('active')) { updatePlayer(); updateStreams(); } }, 5000);";
  ...
  html += "  <div class='menu-item' data-section='playlistEditor' onclick=\"showSection('playlistEditor'); updatePlaylists();\"><span class='menu-icon'>📋</span> Playlist Editor</div>";
  ...
  // === PLAYER SECTION ===
  html += "<div id='player' class='content-section active'>";
  ...
  html += "        <div id='plList2'>";
  {
    ...
    while (file3) {
      ...
      if (name.endsWith(".csv")) {
        ...
        html += "<button class='small' onclick=\"downloadFile('" + name + "')\">⬇️</button>";
        html += "<button class='small' onclick=\"editPlaylist('" + name + "')\">✏️</button>";
  ...
  html += "    <button style='"
          ...
          "font-size:16px;"
          "cursor:pointer;' "
          "onclick='if(confirm(\"Сбросить координаты текста, цвета и стрелки?\")) "
          "fetch(\"/visualreset\").then(()=>alert(\"Сброс выполнен — устройство перезагружается\"));'>"
          "🔄 Reset Visual Settings"
          "</button>";
  html += "    <p style='color:#888;margin-top:8px;font-size:13px'>Сбрасывает только: текстовые координаты, цвета, настройки стрелок.</p>";
  html += "  </div>";

  html += "</div>";  // END upload section
  ...
  html += "<div id='playlistEditor' class='content-section'>";
  html += "  <div class='section'>";
  html += "    <h2>📋 Playlist Editor — <span id='editorTitle'></span></h2>";
  html += "    <div class='playlist-editor'>";
  html += "      <table id='plEditorTable'>";
  html += "        <thead><tr><th>Station Name</th><th>Stream URL</th><th style='width:80px'>Controls</th></tr></thead>";
  html += "        <tbody></tbody>";
  html += "      </table>";
  html += "    </div>";
  html += "    <div style='margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;'>";
  html += "      <button onclick='addRow()'>➕ Add Stream</button>";
  html += "      <button onclick='moveUp()'>🔼 Move Up</button>";
  html += "      <button onclick='moveDown()'>🔽 Move Down</button>";
  html += "      <button onclick='savePlaylist()'>💾 Save</button>";
  html += "      <button onclick=\"showSection('upload')\">❌ Close</button>";
  html += "      <button onclick=\"if(editFilename) downloadFile(editFilename)\">⬇️ Download</button>";
  html += "    </div>";
  html += "    <p style='color:#888;margin-top:10px;font-size:13px'>💡 Кликните на строку чтобы выделить её, затем используйте кнопки 🔼/🔽 для перемещения</p>";
  html += "  </div>";
  html += "</div>";
  ...
  html += "<script>";
  html += "window.addEventListener('load', function(){ updateBackgrounds(); updatePlaylists(); updateStreams(); updatePlayer(); });";
  html += "</script>";
  html += "</body></html>";
