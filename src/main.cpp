...
// --- Needle settings ---
int needleThickness = 2;      // Толщина стрелки
int needleLenMain = 40;       // Длина основной части
int needleLenRed = 20;        // Длина красной части
...
int stationNameX = 10, stationNameY = 60;
int streamTitle1X = 10, streamTitle1Y = 135;
int streamTitle2X = 10, streamTitle2Y = 210;
int bitrateX = 10, bitrateY = 285;
int clockX = 120, clockY = 360;  // X будет пересчитан для центрирования
int volumeX = 200, volumeY = 285;
...
void loadSettings() {
  ...
  // Needle settings
  needleThickness = prefs.getInt("needleThk", 2);
  needleLenMain = prefs.getInt("needleMain", 40);
  needleLenRed = prefs.getInt("needleRed", 20);
  ...
  // Text positions
  stationNameX = prefs.getInt("stationX", 10);
  stationNameY = prefs.getInt("stationY", 60);
  streamTitle1X = prefs.getInt("title1X", 10);
  streamTitle1Y = prefs.getInt("title1Y", 135);
  streamTitle2X = prefs.getInt("title2X", 10);
  streamTitle2Y = prefs.getInt("title2Y", 210);
  bitrateX = prefs.getInt("bitrateX", 10);
  bitrateY = prefs.getInt("bitrateY", 285);
  clockY = prefs.getInt("clockY", 360);
  volumeX = prefs.getInt("volumeX", 200);
  volumeY = prefs.getInt("volumeY", 285);
...
bool connectToWiFi() {
  ...
  if (saved_ssid.length() == 0) {
    ...
    WiFi.mode(WIFI_AP);
    WiFi.softAP("ESP32-Radio-Setup", "12345678");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    wifiConnected = true;  // 👈 добавь эту строку
    return true;
  } else {
...
void drawDataLines() {
  ...
  String line3 = icy_bitrate.length() > 0 ? icy_bitrate + " kbps" : "---";
